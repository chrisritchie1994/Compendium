...
def data_dissemination(sender, instance, **kwargs):
    ...
    for r in x_list:
        if r["record_type"] == 'idea':
            i = Idea(entry=r['idea'], idea=r['idea'], user=instance.user, journal_id=instance)
            i.save()
        elif r["record_type"] == 'decision':
            d = Decision(entry=r['decision'], decision=r['decision'], user=instance.user, journal_id=instance)
            d.save()
        elif r["record_type"] == 'principle':
            p = Principle(entry=r['principle'], principle=r['principle'], user=instance.user, journal_id=instance)
            p.save()
        elif r["record_type"] == 'aphorism':
            a = Aphorism(entry=r['aphorism'], aphorism=r['aphorism'], user=instance.user, journal_id=instance)
            a.save()
